...
@section('title', 'Pembayaran berhasil — Hotel ABC')
...
@section('content')
    ...
    <div class="mx-auto max-w-2xl px-4 py-16 sm:px-6 lg:px-8">
        <div class="rounded-3xl border border-stone-200 bg-white p-8 text-center shadow-md sm:p-10">
            ...
            <h1 class="mt-6 font-serif text-2xl font-semibold text-stone-900">Pembayaran berhasil!</h1>
            <p class="mt-2 text-sm text-stone-600">Referensi: <span class="font-mono font-semibold text-stone-900">{{ $reservation->reference }}</span></p>
            <p class="mt-6 text-left text-sm leading-relaxed text-stone-600">
                Terima kasih, <span class="font-medium text-stone-900">{{ $reservation->guest_name }}</span>. Pembayaran Anda telah dikonfirmasi.
            </p>
            <dl class="mt-8 space-y-3 rounded-2xl bg-canvas px-5 py-4 text-left text-sm">
                <div class="flex justify-between gap-4">
                    <dt class="text-stone-500">Hotel</dt>
                    <dd class="font-medium text-stone-900">{{ $reservation->hotel->name }}</dd>
                </div>
                <div class="flex justify-between gap-4">
                    <dt class="text-stone-500">Kamar</dt>
                    <dd class="font-medium text-stone-900">{{ $reservation->room->type }}</dd>
                </div>
                <div class="flex justify-between gap-4">
                    <dt class="text-stone-500">Menginap</dt>
                    <dd class="font-medium text-stone-900">{{ $reservation->check_in->format('d M Y') }} – {{ $reservation->check_out->format('d M Y') }}</dd>
                </div>
                <div class="flex justify-between gap-4">
                    <dt class="text-stone-500">Tamu</dt>
                    <dd class="font-medium text-stone-900">
                        {{ $reservation->adults }} dewasa{{ $reservation->children > 0 ? ', '.$reservation->children.' anak' : '' }}
                    </dd>
                </div>
                <div class="flex justify-between gap-4 border-t border-stone-200 pt-3">
                    <dt class="text-stone-500">Total dibayar</dt>
                    <dd class="font-semibold text-stone-900">{{ $fmt($reservation->total) }}</dd>
                </div>
                <div class="flex justify-between gap-4">
                    <dt class="text-stone-500">Status pembayaran</dt>
                    <dd class="font-semibold text-emerald-600">Lunas</dd>
                </div>
            </dl>
            @if (! empty($reservation->requests))
                <p class="mt-6 text-left text-sm text-stone-600"><span class="font-medium text-stone-800">Permintaan:</span> {{ $reservation->requests }}</p>
            @endif
